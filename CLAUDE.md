...
└── assets/
    ...
    └── data/
        ...
        ├── level-data.js   レベル詳細 (story, hints, approach, password, fs spec) ※全35レベル執筆済み
...
| 5c | 上位レベル用の追加コマンド (Wave A〜G) ※5b2 より先に実施 | ✅ |
| 5b2 | レベル 11〜34 の詳細・仮想ファイルシステム執筆 | ✅ |
| 6 | 仕上げ (a11y / レスポンシブ / アニメーション微調整) | ✅ |

### Phase 6 で入れた仕上げ

- **a11y**: 全 5 ページに skip-link + `<main id="main">`、進捗バーに `role="progressbar"` と `aria-valuenow/aria-valuemax/aria-valuetext`、ターミナル入力に `role="textbox"`/`aria-multiline`/`aria-describedby`、出力の `aria-live` を off にして別 `role="status"` のライブリージョンに「コマンド完了 / 出力 N 行」だけ要約通知、level-card の `aria-label` をクリア時のみ付与
- **focus / contrast**: nav リンク / disclosure summary / chip / level-card に `:focus-visible` の outline + box-shadow を二重化、`nav__link[aria-current="page"]` に下線、level-card クリア済みに色帯と二重円 (色覚多様性対応)
- **prefers-reduced-motion**: caret 点滅 / クリア演出 / 進捗バーアニメを `prefers-reduced-motion: reduce` で無効化
- **レスポンシブ < 480px**: nav タップターゲット 44px 確保、ターミナル `clamp(320px, 60dvh, 480px)` + `pointer: coarse` で resize ハンドル無効化、パスワードゲートを縦並びにしてボタンをフル幅、sheet-grid を `minmax(min(340px, 100%), 1fr)` で 320px 端末にも対応
- **アニメ**: パスワード正解時に `password-gate--success` クラスで 1.2s の控えめな border-glow、App.toast でクリア通知、SR が読み終わるまで遷移遅延を 900ms → 1400ms に

### Level 11〜34 の通しトレース (5b2)

すべて Chrome ヘッドレスで想定解を再現してパスワード取得まで確認済み (Lv11-34 で 38 件)。
- **11**: ROT13 を tr で復号
- **12**: data.txt (hex dump) → xxd -r → gz → xz → tar → bz2 → gz の 5 段解凍
- **13**: ssh -i sshkey.private bandit14@localhost cat readme
- **14**: cat pw | nc localhost 30000
- **15**: cat pw | openssl s_client -connect localhost:30001 -quiet
- **16**: nmap -sV 30100-30200 → SSL ポートを総当たり → 秘密鍵取得 → ssh
- **17**: diff passwords.old passwords.new
- **18**: ssh user@host cat readme で .bashrc バイパス
- **19**: ./bandit20-do cat /etc/bandit_pass/bandit20 (setuid)
- **20**: cat pw > /tmp/listener-input; ./suconnect 31000 (擬似 listener/connector)
- **21**: /etc/cron.d → スクリプト → 出力ファイル (事前生成) を辿る
- **22**: md5sum で動的ファイル名を計算して /tmp/<hash> を読む
- **23**: /var/spool/cron/bandit24/ にスクリプト設置 → /usr/local/bin/cron-tick (setuid) で実行
- **24**: echo "$(cat pw) 1729" | nc localhost 30002 (1729 はハーディ・ラマヌジャン数のヒント)
- **25**: less welcome.txt → !cat /home/bandit26/.readme で脱出
- **26**: vim welcome.txt → :!cat /home/bandit27/secret
- **27〜31**: git clone / log / show / branch / checkout / tag / add+commit+push
- **32**: 制限シェル (uppercase モード) で $0 → /bin/sh exec → restricted 解除
- **33-34**: 振り返り / 次のステップ案内 (合言葉でクリア)

### 追加で実装した補助コマンド (5b2 中に判明)

- **chmod / chown**: setuid / 鍵パーミッション設定のため
- **md5sum / sha256sum**: Lv22 の md5 ファイル名生成のため (Wave F)
- **shell に $() / `` コマンド置換**: Lv24 等で必須
- **shell に `var=value` 形式の変数代入**: bash 風シェルの基本

### テストハーネス一覧 (合計 172 件)

- `.test-wave-a.html` 〜 `.test-wave-g.html`: 各 Wave の単体・統合テスト (113 件)
- `.test-regression-lv0-10.html`: Lv0〜10 の回帰 (13 件)
- `.test-levels-11-onwards.html`: Lv11〜34 の想定解通しトレース (38 件)
- `.test-a11y.html`: Phase 6 で入れた ARIA 構造の検査 (8 件)
- `.test-help-scroll.html`: help コマンド入力時にページがスクロールしないかの確認
- `.test-compression.py`: Wave A の純アルゴリズムテスト (Python)

### Wave A〜G: 追加コマンドの実装状況 (5c)

すべて検証済み (合計 126/126 ヘッドレステスト pass + Lv0-10 回帰 + help スクロール修正)。
- **A**: gzip/gunzip/zcat/bzip2/bunzip2/bzcat/xz/unxz/xzcat/tar (file 検出も拡張)
- **B**: less/more/vim/vi (shell._interactive と Terminal の対話モード基盤、!cmd / :!cmd / :shell 脱出)
- **C**: nc/openssl s_client/nmap (VFS.ports に仮想ポートサーバを登録、TLS 区別)
- **D**: ssh -i 鍵認証 (VFS.sshKeys、autoExit、`ssh user@host cmd` でリモート remoteFs 実行)
- **E**: setuid 仮想バイナリ (VFS file node の exec 関数 + 0o4000 で所有者切替)、制限シェル ("uppercase" モード) + $0 脱出、parser に $0-$9 追加
- **F**: crontab / md5sum / sha256sum (md5 は純 JS 実装、sha256 は WebCrypto)
- **G**: git clone/log/show/branch/checkout/tag/status/add/commit/push (VFS.gitRepos に仮想リモート、_gitStates Map にクローン状態)
